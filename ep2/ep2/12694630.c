...
//AUXILIARES
double calculaGrauEntradaVertice(Grafo* g, int vertice) {
    double retorno = 0;
    for (int i = 0; i < g->numVertices; i++)
    {
        if (g->matriz[i][vertice] == true && i != vertice)
        {
            retorno++;
        }
    }
...
double calculaGrauSaidaVertice(Grafo* g, int vertice) {
    double retorno = 0;
    for (int i = 0; i < g->numVertices; i++)
    {
        if (g->matriz[vertice][i] == true && i != vertice)
            retorno++;
    }
    return retorno;
}

double centralidadeDeProximidadeAux(Grafo* g, int vertice, int** dist) {
    double soma = 0;
    for (int i = 0; i < g->numVertices; i++) {
        soma += dist[vertice][i];
    }
    
    return ((double)g->numVertices-1)/soma;
}

double centralidadeDeIntermediacaoAux(Grafo* g, int vertice, int** pred) {
    double resultado = 0;
    int n = g->numVertices;
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < n; j++) {
            if (j == vertice || j == i) continue;

            int atual = j;
            while (atual != i && atual != -1) { // O atual não é o destino i e não é -1 (indicando que não tem caminho)
                atual = pred[i][atual];
    ...
    return resultado/ ((n-1)*(n-2));
}

double pageRankAux(Grafo* g, int n, int vertice, double* valores,double d)
{
    double resultado = 0;
    for (int i = 0; i < n; i++)
    {
        if (g->matriz[i][vertice] == true && i != vertice)
        {
            resultado += valores[i] / calculaGrauSaidaVertice(g, i);
        }
    }
...
void centralidadeDeGrau(Grafo* g, double* valores) {
    ...
    for (int i = 0; i < g->numVertices; i++)
    {
        valores[i] = calculaGrauEntradaVertice(g, i) / (double)(g->numVertices-1);
    }
}
...
void centralidadeDeProximidade(Grafo* g, double* valores) {
    ...
    int x;
    int** pred = (bool**)malloc(sizeof(bool*) * g->numVertices);
    int** dist = (bool**)malloc(sizeof(bool*) * g->numVertices);

    for (x = 0; x < g->numVertices; x++) {
        pred[x] = (bool*)malloc(sizeof(bool) * g->numVertices);
        dist[x] = (bool*)malloc(sizeof(bool) * g->numVertices);
    }

    calculaDistanciaFloydWarshall(g, dist, pred);
    //exibeMatrizDistancias(dist, g->numVertices);
    //exibeMatrizPredecessores(pred, g->numVertices);

    for (int i = 0; i < g->numVertices; i++)
    {
        valores[i] = centralidadeDeProximidadeAux(g, i, dist);
    }
    free(pred);
    free(dist);
}
...
void centralidadeDeIntermediacao(Grafo* g, double* valores) {
    ...
    for (int i = 0; i < g->numVertices; i++)
    {
        valores[i] = centralidadeDeIntermediacaoAux(g, i, pred);
    }
...
void centralidadePageRank(Grafo* g, double* valores, int iteracoes) {
    ...
    double d = 0.85;
    int n = g->numVertices;
    double* valoresAntigos = (double*)malloc(sizeof(double) * n);
    for (int i = 0; i < n; i++) 
    {
        valores[i] = (double)1 / (double)n;
        valoresAntigos[i] = valores[i];
    }

    for (int i = 0; i < iteracoes; i++)
    {
        for (int j = 0; j<n;j++)
            valores[j] = ((double) 1 - d)/n + d * pageRankAux(g,n,j,valoresAntigos,d);

        for (int j = 0; j < n;j++)
            valoresAntigos[j] = valores[j];
    }
    free(valoresAntigos);
}
